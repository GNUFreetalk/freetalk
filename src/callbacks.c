...
LmSSLResponse
ft_ssl_response_cb (LmSSL *ssl, LmSSLStatus st, gpointer user_datta)
{
  /* Dont really care.  */
      ...
      if (!g_ascii_strcasecmp (x->name, "x"))
        {
          ...
          if (xmlns && !g_ascii_strcasecmp (xmlns, "jabber:x:delay"))
            {
              ts =
                parse_timestamp ((char *)
                                 lm_message_node_get_attribute (x, "stamp"));
            }
        }
    }
                        ...
                        LmDisconnectReason reason, gpointer user_data)
{
  ...
  scm_run_hook (ex_disconnect_hook, scm_list_n (scm_from_int (reason),
                                                SCM_UNDEFINED));
  /* Set conn_status after hook so that discon hook procedures can get the
     previous state from ft-get-conn-status.  Helps in deciding if an
     automatic reconnection logic (dont auto reconnect if previous state was
     not FT_AUTH, etc).  */
  do_set_conn_status (FT_DEAD);
...
ft_register_msg_handlers (LmConnection *conn)
{
  LmMessageHandler *handler =
    lm_message_handler_new ((LmHandleMessageFunction) ft_msg_msg_handler,
                            NULL, NULL);
  ...
  handler =
    lm_message_handler_new ((LmHandleMessageFunction) ft_msg_presence_handler,
                            NULL, NULL);
  lm_connection_register_message_handler (conn, handler,
                                          LM_MESSAGE_TYPE_PRESENCE,
                                          LM_HANDLER_PRIORITY_NORMAL);
  ...
  handler =
    lm_message_handler_new ((LmHandleMessageFunction) ft_msg_iq_handler, NULL,
                            NULL);
  lm_connection_register_message_handler (conn, handler, LM_MESSAGE_TYPE_IQ,
                                          LM_HANDLER_PRIORITY_NORMAL);
  lm_message_handler_unref (handler);

  lm_connection_set_disconnect_function (conn, ft_disconnect_function, NULL,
                                         NULL);
}
