...
/* Return the subscription state for a roster item.  */
...
/* Lookup a roster item based on the JID.  The 'resource' part of the JID is
   ignored for comparision purposes.  */
FtRosterItem *
  ...
  if (old)
    {
      ...
      if (!g_ascii_strcasecmp
          (lm_message_node_get_attribute (item, "subscription"), "remove"))
        {
...
/* Remove a buddy from the roster and cancel all subscriptions (from and
   to).  */
void
