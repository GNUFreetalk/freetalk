...
do_disconnect ()
{
  /* if (!do_get_conn_status ()) return -1; */
...
{
  do_disconnect ();
  scm_run_hook (ex_quit_hook, scm_list_n (scm_from_int (err), SCM_UNDEFINED));
  exit (err);
}
...
{
  /* set before hook, allowing hook functions to override default buddy */
  ...
  scm_run_hook (ex_message_send_hook,
                scm_list_n (scm_from_locale_string (jid),
                            scm_from_locale_string (msg_str), SCM_UNDEFINED));
...
{
  const char *valid[] =
    { "online", "away", "chat", "xa", "dnd", "invisible", NULL };
  char *text, *priority;
  int show, offset;
...
static int
strcmp_rev (gpointer a, gpointer b)
{
...
do_dict_insert (char *word)
{
  if (!g_slist_find_custom (dict_words, word, (GCompareFunc) strcmp_rev))
    dict_words =
      g_slist_insert_sorted (dict_words, word, (GCompareFunc) strcmp_rev);
}
...
{
  GSList *tmp =
    g_slist_find_custom (dict_words, word, (GCompareFunc) strcmp_rev);
